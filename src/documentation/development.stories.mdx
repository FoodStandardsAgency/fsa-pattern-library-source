...
<Meta title="Documentation/2 - Developer Workflow" />
...
Use snake_case for the variables in the twig templates.
...
To make sure each component is independent and modular, the SVG files a component uses should be placed in an `svg` folder at the component's level.